"""
Purchase Invoice UI for PlywoodPro.
Mirrors the Sales Invoice layout but for supplier purchases.
On confirm: triggers stock IN and ITC journal entries.
"""
...
from modules.purchase import (
    create_purchase_invoice, confirm_purchase_invoice, get_purchase_invoices
)
...
class PurchaseInvoiceUI(ctk.CTkFrame):
    ...
    def _build_ui(self):
        """Build the complete purchase invoice form."""
        # Title
        title_frame = ctk.CTkFrame(self, fg_color="transparent")
        title_frame.pack(fill="x", padx=10, pady=(10, 5))
        ctk.CTkLabel(
            title_frame, text="Purchase Invoice",
        ...
        ctk.CTkLabel(r1, text="Supplier:", font=ctk.CTkFont(weight="bold")).pack(side="left")
        ...
        ctk.CTkLabel(r2, text="Supplier Bill No:").pack(side="left")
        self.ref_var = ctk.StringVar()
        ctk.CTkEntry(r2, textvariable=self.ref_var, width=180).pack(side="left", padx=5)
        ...
        ctk.CTkButton(
            add_row_frame, text="+ Add Row", fg_color="#1565C0",
            hover_color="#0D47A1", width=100, command=self._add_row,
        ...
        ctk.CTkButton(
            btn_frame, text="✅ Confirm & Book Stock", fg_color=ACCENT,
            hover_color=ACCENT_HOVER, width=180, command=self._confirm_and_book,
        ).pack(side="left", padx=5)
    ...
    def _load_dropdowns(self):
        """Load suppliers, items, godowns into dropdowns."""
        self.party_list = get_all_parties(party_type='supplier')
        ...
        if item_names:
            ...
            self.rate_var.set(str(self.item_list[0].get('purchase_rate', 0)))
    ...
    def _on_party_changed(self, *args):
        """Detect intra/inter-state when supplier changes."""
        ...
        if is_intra_state(company_code, party_code):
            self.tax_label.configure(
                text="Tax: CGST + SGST (Intra-state) — ITC Eligible",
                text_color="#FF9800"
            )
        else:
            self.tax_label.configure(
                text="Tax: IGST (Inter-state) — ITC Eligible",
                text_color="#E91E63"
            )
    ...
    def _refresh_tree(self):
        """Refresh treeview from self.line_items."""
    ...
    def _recalculate_all_rows(self):
        """Recalculate all rows when supplier changes."""
        party = self._get_selected_party()
        company_code = self.company.get('state_code', '') if self.company else ''
        party_code = party.get('state_code', '') if party else ''
    ...
    def _save_draft(self):
        ...
        success, message, vid = create_purchase_invoice(header, self.line_items)
    ...
    def _confirm_and_book(self):
        """Save, confirm (books stock + ITC), and show result."""
        valid, msg = self._validate_form()
        if not valid:
            CTkMessagebox(title="Validation Error", message=msg, icon="cancel")
            return
        ...
        ok, cmsg = confirm_purchase_invoice(vid)
        if ok:
            CTkMessagebox(
                title="Success",
                message=f"Purchase Invoice confirmed.\nStock updated. ITC booked.",
                icon="check"
            )
        else:
